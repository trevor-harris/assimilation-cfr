...
for (i in 1:100) {
  ...
  sim.prior[,,i] = sim.prior[,,i] * 10000 * matrix(rnorm(100*100, 25, 10), 100, 100)
}
...
for (i in 1:100) {
  sim.post[,,i] = runif(1, 0.75, 1.25) * c(outer(dnorm((1:100), 25, 25), dnorm((1:100), 75, 25), "*")) + 
                  runif(1, 0.75, 1.25) * c(outer(dnorm((1:100), 75, 25), dnorm((1:100), 25, 25), "*"))
  sim.post[,,i] = sim.post[,,i] * 10000 * matrix(rnorm(100*100, 25, 10), 100, 100)
}
...
plot(perm.fields[,1], type = "l", main="Noisy Test - Same", ylab = "Mean Wilcoxon", xlab = "Region", ylim = c(min(perm.fields), max(perm.fields)))
for(p in 2:perms) {
  lines(perm.fields[,p])
}
...
# plot the observed and permuted (as vectors)
plot(perm.fields[,1], type = "l", main="Noisy Test - Different", ylab = "Mean Wilcoxon", xlab = "Region", ylim = c(min(perm.fields), max(perm.fields)))
for(p in 2:perms) {
  lines(perm.fields[,p])
}
lines(wilco.field, col = "red", lwd = 2)
