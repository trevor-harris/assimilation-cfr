...
plot(perm.fields[,1], type = "l", main="Smooth Test - Same", ylab = "Mean Wilcoxon", xlab = "Region", ylim = c(min(perm.fields), max(perm.fields)))
for(p in 2:perms) {
  lines(perm.fields[,p])
}
...
# plot the observed and permuted (as vectors)
plot(perm.fields[,1], type = "l", main="Smooth Test - Different", ylab = "Mean Wilcoxon", xlab = "Region", ylim = c(min(perm.fields), max(perm.fields)))
for(p in 2:perms) {
  lines(perm.fields[,p])
}
lines(wilco.field, col = "red", lwd = 2)
