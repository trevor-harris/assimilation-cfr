...
for (scales in c(0.01, 0.5, 5)) {
  prior.gp = sim_gp(mu = 0, scale = 1)
  post.gp = sim_gp(mu = 0, scale = scales)
  ...
  plot(perm.fields[,1], type = "l", main=paste0("Under Alternative scale = ", scales), ylab = "Mean WILCOX", xlab = "Region", ylim = c(pltmin, pltmax))
  for(p in 2:perms) {
    lines(perm.fields[,p])
  }
  ...
  plot(perm.fields[,1], type = "l", main=paste0("Under Alternative scale = ", scales), ylab = "Mean K-S", xlab = "Region", ylim = c(pltmin, pltmax))
  for(p in 2:perms) {
    lines(perm.fields[,p])
  }
