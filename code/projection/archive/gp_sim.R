...
post.gp = sim_gp(mu = 0.1, scale = 1)

# library(plotly)
# plot_ly(showscale = F) %>%
#   add_surface(z = ~prior.gp[,,1]) %>%
#   add_surface(z = ~prior.gp[,,2]) %>%
#   add_surface(z = ~prior.gp[,,3]) %>%
#   add_surface(z = ~prior.gp[,,4]) %>%
#   add_surface(z = ~post.gp[,,1]) %>%
#   add_surface(z = ~post.gp[,,2]) %>%
#   add_surface(z = ~post.gp[,,3]) %>%
#   add_surface(z = ~post.gp[,,4])
...
plot(perm.fields[,1], type = "l", main="Under Alternative", ylab = "Mean WILCOX", xlab = "Region", ylim = c(pltmin, pltmax))
for(p in 2:perms) {
  lines(perm.fields[,p])
}
...
plot(perm.fields[,1], type = "l", main="Under Alternative", ylab = "Mean K-S", xlab = "Region", ylim = c(pltmin, pltmax))
for(p in 2:perms) {
  lines(perm.fields[,p])
}
