...
basis = irlba(flatten(prior), 100)$u

test = t(basis) %*% as.vector(prior[,,1])
plot(test)

basis1 = irlba(flatten(prior), 100)

for(t in c(1, 250, 500, 750, 998)) {
  post = prep_post(nc.post, t)
  
  prior.t = sapply(1:dim(prior)[3], function(x) proj %*% as.vector(prior[,,x]))
  post.t = sapply(1:dim(post)[3], function(x) proj %*% as.vector(post[,,x]))
  
  # test
  cat(ks.mean(prior.t, post.t), "\n")
}
