...
size = data.frame(stat = c(k.xd[,1], k.wa[,1], q.xd[,1]),
                  pval = c(k.xd[,2], k.wa[,2], q.xd[,2]),
		  method = rep(c("K_XD", "K_WA", "Q_XD"), each=sims),
                  sims = sims,
                  pts = pts,
                  functions = n,
                  range = l,
...
saveRDS(size, file = paste0("/home/trevorh2/size/output_warp/sim",i,".RDS"))
