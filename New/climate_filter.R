...
post = prep_post(nc.post, 800)

# cheap viz
image.plot(rotateFixed(post[,,2], 270))
image.plot(rotateFixed(down_sample_image(prior[,,2], 4), 270))

# convert matrix representation into vector rep
prior = sapply(1:dim(prior)[3], function(x) down_sample_image(prior[,,x], 3))
post = sapply(1:dim(post)[3], function(x) down_sample_image(post[,,x], 3))
