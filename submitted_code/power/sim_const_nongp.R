...
for (mu2 in seq(-1, 1, by = 0.1)) {
  ...
  vals = future_sapply(1:sims, function(x) {
    ...
    # FAD often fails with heavy tails. Use NA to represent missing values in these cases.
  ...
  saveRDS(power, file = paste0("power/out_const_nongp/",feature,"_sim_",i,"_const",".RDS"))
...
for (sd2 in seq(0.1, 2, by = 0.05)) {
  ...
  vals = future_sapply(1:sims, function(x) {
    f = abs(tp2d(fields = n1, mu = mu1, sd = sd1, range = r1, nu = nu1, pts = pts))
    g = abs(tp2d(fields = n2, mu = mu2, sd = sd2, range = r2, nu = nu2, pts = pts))
